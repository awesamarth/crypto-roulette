...
export default function Home() {
  ...
  const playerPositions = Array.from({ length: roomData.playersArray?.length }, (_, i) => {
    ...
    <main className="flex min-h-screen flex-col items-center justify-between">
      {roomConnected?
        ...
        <Canvas className="border-2">
          {/* Point light positioned higher above the ground */}
          <ambientLight intensity={0.5} />
          <pointLight position={[0, 7, 0]} intensity={100} />
          <Physics>
            {playerPositions.map((position, index) => (
              <Player key={index} initialPosition={position} isMyTurn={roomData.whoseTurn===userId} isMe={userId==roomData.playersArray[index]} />
            ))}
            <Text
              position={[0, 2, 0]} // Position of the text in the 3D scene
              fontSize={1} // Size of the text
              color="green" // Text color
              anchorX="center" // Text alignment on the X axis
              anchorY="middle" // Text alignment on the Y axis
              rotation="true"
            >
              Crypto Roulette
            </Text>
