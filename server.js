...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
    ...
    function randomElementsGetter(array) {
      let numberOfDetonateAllers = Math.floor(array.length / 2);
      let randomElements = [];
      let tempArray = [...array];
      for (let i = 0; i < numberOfDetonateAllers; i++) {
    ...
    function detonateRandomPlayers(roomId) {
      ...
      for (let i = 0; i < numPlayersToEliminate; i++) {
        let randomIndex = Math.floor(Math.random() * roomData[roomId].activePlayersArray.length);
